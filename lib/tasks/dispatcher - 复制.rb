...
begin
  ...
  begin  
	...
	puts "ftp.nlst('/TCS/Download') error"
  end
  puts "close ftp"
  ftp.close
rescue  SystemCallError => e
  ...
  #puts "time: #{Time.now} error:#{$!} at:#{$@}"
  puts "vpn not connected"
  test = system('C:\Program Files\Sangfor\SSL\SangforCSClient\SangforCSClient.exe',' /ShortCutAutoLogin')
  puts "reconnecting vpn : #{test}"
  File.open(rails_root + '/log/ftp.log', 'a') do |file|
    file.puts "time: #{Time.now} reconnecting vpn : #{test}"
  end
end
puts 'end receive'
